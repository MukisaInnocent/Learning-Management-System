...
export class AttendanceService {
  ...
  async markAttendance(records: { studentId: string; termId: string; date: string; status: AttendanceStatus; notes?: string }[]) {
    ...
    for (const r of records) {
      ...
      const result = await this.prisma.attendance.upsert({
        ...
        create: { studentId: r.studentId, termId: r.termId, date, status: r.status, notes: r.notes },
      });
      results.push(result);
    }
    return results;
  ...
  async getAttendanceForDate(organizationId: string, date: string, termId: string) {
  ...
  async getSummary(studentId: string, termId: string) {
    const records = await this.prisma.attendance.findMany({ where: { studentId, termId } });
