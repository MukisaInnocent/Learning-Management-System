...
export class AttendanceController {
  ...
  @Get('date')
  @Roles(Role.SUPER_ADMIN, Role.ORG_ADMIN, Role.TEACHER)
  getForDate(@CurrentUser() user: any, @Query('date') date: string, @Query('termId') termId: string) {
    return this.attendanceService.getAttendanceForDate(user.organizationId, date, termId);
  }

  @Get('students/:studentId')
  getStudentAttendance(@Param('studentId') studentId: string, @Query('termId') termId?: string) {
  ...
  @Get('me')
  @Roles(Role.STUDENT)
  getMyAttendance(@CurrentUser() user: any, @Query('termId') termId?: string) {
